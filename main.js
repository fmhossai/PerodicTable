import {Create, insertData, blnk,blnk2,choose,showElements} from "./CreateTable.js"
...
function ready(){
  ...
  inputfield.addEventListener("keyup",(e)=>{
    ...
    if(searchStr.length === 1){
      showElements(symbols.filter(symbol => symbol === searchStr), symbols)
    }
    else if(searchStr === ""){
        showElements(symbols,symbols)
    }
    else{
      let filteredElements = symbols.filter(symbol => dict[symbol].includes(searchStr))
